 ...
 *
 * This class defines the basic functionality that every Node will have.
 * It supports polymorphism, allowing different types of nodes to define their own behaviors.
 */
//-------------------------------------------------------------------
class Node 
{
public:
    ...
    /**
     *  @brief Virtual destructor for Node to allow derived class destructors to be called.
     */
    virtual ~Node();
    ...
    virtual const std::string& get_node_type()const = 0;

    virtual int get_number_of_input_pins()const = 0;
    virtual int get_number_of_output_pins()const = 0;
     ...
     */
    Pin<DataType>* find_pin_using_id(int pin_id)
    {
